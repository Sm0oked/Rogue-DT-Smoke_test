...
local menu_elements =
{
    ...
    min_hits              = slider_int:new(1, 20, 1, get_hash(my_utility.plugin_label .. "pen_shot_base_min_hits_to_casttrap_base_pos")),
    ...
    boss_override = checkbox:new(true, get_hash(my_utility.plugin_label .. "boss_override_pen_shot")),
}
...
local function menu()
    ...
    if menu_elements.tree_tab:push("Penetrating Shot") then
        ...
        menu_elements.boss_override:render("Boss Override", "Cast on bosses regardless of hit count");
...
local function logics(entity_list, target_selector_data, best_target)
    ...
    local boss_override = menu_elements.boss_override:get();
    ...
    if best_target and best_target:is_boss() and boss_override then
        ...
        if not is_wall_collision then
            if cast_spell.position(spell_id_penetration_shot, cast_position, 0.4) then
                ...
                next_time_allowed_cast = current_time + 8.0;
                    
                console.print("Rouge Plugin, Casted Penetrating Shot on Boss - Close Quarter Combat activation");
                return true;
            end
        end
        return false;
    ...
    -- Remove debug logging that could spam console
    ...
    if cast_spell.position(spell_id_penetration_shot, cast_position, 0.4)then
        ...
        console.print("Rouge Plugin, Casted Penetrating Shot - Close Quarter Combat activation");
        return true;
    end
    return false;
end
