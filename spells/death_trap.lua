...
local menu_elements_death_trap =
{
    ...
    boss_override = checkbox:new(true, get_hash(my_utility.plugin_label .. "boss_override_death_trap")),
}
...
local function menu()
    ...
    if menu_elements_death_trap.tree_tab:push("Death Trap") then
        ...
        menu_elements_death_trap.boss_override:render("Boss Override", "Cast on bosses regardless of hit count");
...
local function logics(entity_list, target_selector_data, best_target)
    ...
    local boss_override = menu_elements_death_trap.boss_override:get();
    ...
    if best_target and best_target:is_boss() and boss_override then
        ...
        if cast_spell.position(death_trap_spell_id, best_target:get_position(), 0.40) then
            local current_time = get_time_since_inject();
            next_time_allowed_cast = current_time + 0.1; -- Very short cooldown to allow spamming
                
            console.print("Rouge Plugin, Casted Death Trap on Boss (Spam Priority)");
            return true;
        end
        return false;
    end
    ...
    if cast_spell.position(death_trap_spell_id, cast_position, 0.40)then
        ...
        console.print("Rouge Plugin, Casted Death Trap (Spam Priority)");
        return true;
    end
